...
var REPO_CONFIGS = [
  ...
  { repo: "backfill", workflow: "backfill.yml", branch: "master" }
];
...
async function executeRestart(config, state, headers, env) {
  ...
  const body = repo === "backfill" ? JSON.stringify({ ref: branch }) : JSON.stringify({ ref: branch, inputs: { triggered_by: "workflow-watchman" } });
